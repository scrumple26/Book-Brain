...
export default function BookPage() {
  ...
  function saveChapterName(chapterId: string) {
    ...
    const updated = {
      ...
      chapters: book.chapters.map((c) =>
        c.id === chapterId ? { ...c, name: editingChapterName.trim() } : c
      ),
    };
    persist(updated);
  ...
  function addNote() {
    ...
    const updated = {
      ...book,
      chapters: book.chapters.map((c) =>
        c.id === activeChapterId ? { ...c, notes: [...c.notes, note] } : c
      ),
    };
    persist(updated);
    setNoteInput("");
      ...
      <header className="border-b border-parchment-300 bg-parchment-100 px-6 py-4 flex-shrink-0">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => router.push("/")}
              className="text-ink-300 hover:text-amber-600 transition-colors text-sm flex-shrink-0"
            >
              ← Library
            </button>
            <span className="text-parchment-300">|</span>
            <div className="min-w-0">
              <h1 className="font-serif font-semibold text-ink-900 text-lg leading-tight truncate">
                {book.title}
              </h1>
            ...
            {searchResults.length === 0 ? (
              ...
              <div className="space-y-2">
                {searchResults.map((r, i) => (
                  <button
                    key={i}
                    onClick={() => { setActiveChapterId(r.chapterId); setSearch(""); }}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {r.noteText && (
                        <p className="text-sm text-ink-700">{highlight(r.noteText, search)}</p>
                      )}
                    </div>
                  </button>
                ))}
          ...
          <div className="p-4 border-b border-parchment-300">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-amber-600 hover:text-amber-500 text-xl leading-none"
              >
                +
              </button>
            </div>
            ...
            <div className="px-3 pt-3">
              ...
              <div className="flex gap-2 mt-2">
                <button
                  ...
                  className="flex-1 bg-amber-600 disabled:opacity-40 text-white text-xs py-1.5 rounded-md"
                >
                  Add
                </button>
                <button
                  onClick={() => setAddingChapter(false)}
                  className="flex-1 border border-parchment-300 text-ink-500 text-xs py-1.5 rounded-md"
                >
                  Cancel
                </button>
              </div>
                ...
                {editingChapterId === chapter.id ? (
                  <div className="px-3 py-1">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") saveChapterName(chapter.id);
                  ...
                  >
                    ...
                    <span className={`text-xs flex-shrink-0 ${activeChapterId === chapter.id ? "text-amber-100" : "text-ink-300"}`}>
                ...
                {editingChapterId !== chapter.id && (
                  <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex gap-0.5">
                    ...
                    <button
                      ...
                      className={`p-1 rounded text-xs ${activeChapterId === chapter.id ? "text-amber-100 hover:text-white hover:bg-amber-500" : "text-ink-300 hover:text-red-500 hover:bg-parchment-300"}`}
                    >
                      ×
                    </button>
                  </div>
              ...
              <div>
                ...
                <p className="font-serif italic text-ink-500 text-lg">
                  {book.chapters.length === 0
                    ? "Add your first chapter to start taking notes."
            ...
            <div className="flex flex-col h-full">
              <div className="px-8 py-5 border-b border-parchment-200">
                ...
                <p className="text-ink-300 text-xs mt-0.5">
                  {activeChapter.notes.length} note{activeChapter.notes.length !== 1 ? "s" : ""}
                </p>
              </div>
                  ...
                  <ul className="space-y-1.5">
                    {activeChapter.notes.map((note) => (
                      <li key={note.id} className="group flex items-start gap-3">
                        <span className="text-amber-600 mt-1 flex-shrink-0 text-base leading-tight select-none">•</span>
                        ...
                        {editingNoteId !== note.id && (
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                            <button
                              ...
                              className="text-ink-300 hover:text-ink-700 text-xs p-0.5"
                            >
                              ✎
                            </button>
                            <button
                              onClick={() => deleteNote(activeChapter.id, note.id)}
                              className="text-ink-300 hover:text-red-500 text-sm p-0.5 leading-none"
                            >
                              ×
                            </button>
                          </div>
              ...
              <div className="px-8 py-4 border-t border-parchment-200 bg-parchment-50">
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-white border border-parchment-300 rounded-lg px-4 py-2.5 text-sm text-ink-900 placeholder-ink-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                  />
                  <button
                    onClick={addNote}
                    disabled={!noteInput.trim()}
                    className="bg-amber-600 hover:bg-amber-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium px-4 py-2.5 rounded-lg transition-colors"
                  >
                    Add
                  </button>
                </div>
