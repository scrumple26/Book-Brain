...
const PROMPT_PREFIX =
  "Clean up this voice-dictated note. Add a period at the end of every complete sentence, including sentences in the middle of the text — do not leave any sentence boundary without a period. Be aggressive about adding commas: after introductory phrases, before coordinating conjunctions (and, but, or, so, yet), between list items, after transitional words (however, therefore, finally, also, etc.), and wherever a natural spoken pause would occur. Capitalize the first word of every sentence. Fix grammar errors. Keep every word — do not remove, omit, or summarize any content. Output only the corrected text, with no quotes, no labels, no explanation.\n\nNote:\n";
